...
threadpool; LLM calls dominate latency, DB is local sqlite3):
    ...
    POST /api/catalog/{role_id}/distill      (admin-token-gated)

All reads delegate to `app.services.card_service`; this router is a thin
HTTP adapter — validation, 404s, admin gating. The router is registered by
...
@router.post("/api/catalog/{role_id}/distill")
def distill_role(role_id: str, request: Request) -> dict:
    """ADMIN-GATED: force-distill a role's responsibilities synchronously.

    Requires `Authorization: Bearer <APP_ADMIN_TOKEN>`. 401 if missing/wrong.
    Returns {status, distilled} where status is card_service's distill result
    ('done'|'failed'|…). role_id validated to int (422 on bad).
    """
    rid = _parse_role_id(role_id)
