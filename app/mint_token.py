"""CLI for issuing / listing / revoking closed-beta access tokens.
    ...
    python -m app.mint_token <label> [--quota N]   # mint a new token
...
def _cmd_mint(label: str, quota: int | None) -> None:
    token = auth.mint(label, daily_quota=quota)
    print(f"label: {label}")
    print(f"quota: {quota if quota is not None else 'unlimited'}")
    print(f"token: {token}")
...
def main() -> None:
    ...
    _cmd_mint(args.label, args.quota)
